  ...
  <div class="flex min-h-[60vh] items-center justify-center">
    <div class="w-full max-w-md space-y-8 rounded-[2.5rem] bg-white p-10 shadow-xl ring-1 ring-slate-100">
      <div class="text-center">
        ...
        <p class="mt-2 text-slate-500">Login to manage your bookings</p>
      </div>

      <form @submit.prevent="handleLogin" class="space-y-6">
        <div class="space-y-2">
          <label class="text-xs font-bold uppercase tracking-widest text-slate-400">Email Address</label>
          <input v-model="email" type="email" required class="w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 outline-none focus:ring-2 focus:ring-blue-500/20 transition-all" placeholder="admin@lumina.com" />
        </div>
        <div class="space-y-2">
        ...
        <button type="submit" :disabled="loading" class="w-full rounded-2xl bg-blue-600 py-4 text-lg font-bold text-white shadow-lg shadow-blue-200 transition-all hover:bg-blue-700 active:scale-[0.98] disabled:opacity-50">
          {{ loading ? 'Signing in...' : 'Sign In' }}
        </button>
      </form>
      
      <p class="text-center text-sm text-slate-400">
        Don't have an account? <NuxtLink to="/register" class="font-bold text-blue-600">Register</NuxtLink>
      </p>
    </div>
  </div>
</template>
